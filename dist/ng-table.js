(function (angular, factory) {
  ...
  (function () {
    /**
     * @ngdoc directive
     * @name ngTableIncludeReplace
     * @module ngTable
     * @description 使用ng-include包裹后, 会导致其失去table布局, 引入该指令, 在引入模板后移除外层ng-include标签
     */
    angular.module('ngTable')
      .directive('ngTableIncludeReplace', function () {
        return {
          require: 'ngInclude',
          restrict: 'A', /* optional */
  ...
  (function () {
     ...
     *
     * Out of the box the `ngTableDefaultGetData` service will be configured to use the angular `filter` and
     *   `orderBy` filters respectively
     */
  ...
  (function () {
     ...
     */
    angular.module('ngTable').factory('ngTableColumn', [
      function () {
         ...
         * @param {Scope} defaultScope the $scope to supply to the $column getter methods when not supplied by
         *   caller
         * @param {Array} columns a reference to the columns array to make available on the context supplied to
         *   the
        ...
        function buildColumn(column, defaultScope, columns) {
          ...
          for (var prop in defaults) {
            ...
            (function (prop1) {
              // satisfy the arguments expected by the function returned by parsedAttribute in the
              // ngTable directive
  ...
  (function () {
    ...
    angular.module('ngTable').factory('NgTableParams', [
      '$q', '$log', '$filter', 'ngTableDefaults', 'ngTableGetDataBcShim', 'ngTableDefaultGetData', 'ngTableEventsChannel', function ($q, $log, $filter, ngTableDefaults, ngTableGetDataBcShim, ngTableDefaultGetData, ngTableEventsChannel) {
        var isNumber = function (n) {
          return !isNaN(parseFloat(n)) && isFinite(n);
        };
        var NgTableParams = function (baseParameters, baseSettings) {

          // the ngTableController "needs" to create a dummy/null instance and it's important to know whether
          // an instance is one of these
          ...
          var self = this,
            ...
            defaultFilterOptions = {
              ...
              filterDelay: 500,
              filterDelayThreshold: 10000, // size of dataset array that will trigger the filterDelay
                                           // being applied
              filterFilterName: undefined, // when defined overrides
                                           // ngTableDefaultGetDataProvider.filterFilterName
              filterFn: undefined, // when defined overrides the filter function that
                                   // ngTableDefaultGetData uses
           ...
           * 'true': to return the current filter minus any insignificant values (null,  undefined and empty
           *   string); or
           ...
           * @description If 'group' parameter is not set, return current grouping. Otherwise set current
           *   group.
           *
           ...
           * @description If 'sorting' parameter is not set, return current sorting. Otherwise set current
           *   sorting.
           *
           ...
           *
           * When no arguments supplied, the current parameter state of this `NgTableParams` instance will be
           *   used
           *
           ...
           * @description Return true when a change to this `NgTableParams` instance should require the
           *   reload method to be run so as to ensure the data presented to the user reflects the
           *   `NgTableParams`
           */
          this.isDataReloadRequired = function () {
            // note: using != as want to treat null and undefined the same
            return !isCommittedDataset || !angular.equals(createComparableParams(),
                prevParamsMemento) || hasGlobalSearchFieldChanges();
          };
           ...
           * @returns {Array} If asString = true will be return array of url string parameters else key-value
           *   object
           */
           ...
           * @description Return true when an attempt to `reload` the current `parameter` values have
  ...
  (function () {
     ...
     */
    angular.module('ngTable').controller('ngTableController', [
      '$scope', 'NgTableParams', '$timeout', '$parse', '$compile', '$attrs', '$element',
      ...
      function ($scope, NgTableParams, $timeout, $parse, $compile, $attrs, $element, ngTableColumn, ngTableEventsChannel) {
        ...
        // CRITICAL: the watch must be for reference and NOT value equality; this is because NgTableParams
        // maintains the current data page as a field. Checking this for value equality would be terrible for
        // performance and potentially cause an error if the items in that array has circular references
        ...
        this.compileDirectiveTemplates = function () {
          if (!$element.hasClass('ng-table')) {
            ...
            if (!theadFound) {
              headerTemplate = angular.element(document.createElement('thead')).attr('ng-include',
                'templates.header');
        ...
        this.loadFilterData = function ($columns) {
          angular.forEach($columns, function ($column) {
            ...
            // otherwise, we just return what the user gave us. It could be a function, array, object,
            // whatever
            else {
        ...
        this.setupBindingsToInternalScope = function (tableParamsExpr) {
          ...
          // $watch for value equivalence. This is because ngTableParams references the current page of data
          // as a field and it's important not to watch this
        ...
        function setupGroupRowBindingsToInternalScope() {
          ...
          if ($attrs.showGroup) {
            ...
            if (showGroupGetter.assign) {
              // setup two-way databinding thus allowing ngTableGrowRow to assign to the showGroup
              // expression
        ...
        return {
          ...
          compile: function (element, attr) {
            var columns = [],
              ...
              groupRow,
              rows = [],
              trSelector = attr.trClass || 'tr';

            // angular.forEach(element.find('tr'), function(tr) {
            angular.forEach(element.find(trSelector), function (tr) {
            ...
            angular.forEach(dataRow.find('td'), function (item) {
              ...
              // NOTE TO MAINTAINERS: if you add extra fields to a $column be sure to extend
              // ngTableColumn with a corresponding "safe" default
              ...
              columns.push({
                ...
                width: parsedAttribute('width')
              });

              if (groupRow || el.attr("ng-if")) {
                // change ng-if to bind to our column definition which we know will be writable
                // because this will potentially increase the $watch count, only do so if we already
                // have an ng-if or when we definitely need to change visibility of the columns.
  ...
  (function () {
     ...
     */
    angular.module('ngTable').directive('ngTableDynamic', [
      function () {
        ...
        return {
          ...
          compile: function (tElement) {
            ...
            angular.forEach(tElement.find('tr'), function (tr) {
              // angular.forEach(tElement.find('.lv-item'), function(tr) {
  ...
  (function () {
     ...
     * @restrict A
     */
    angular.module('ngTable').directive('ngTablePagination', [
  ...
  (function () {
    ...
    function ngTableFilterRowController($scope, ngTableFilterConfig) {
      ...
      $scope.getTmpUrl = function () {
        return $scope.template || 'ng-table/filterRow.html';
      };
  ...
  (function () {
    ...
    function ngTableFilterRow() {
      var directive = {
        ...
        // 因为必须将tr直接包含thead内，否则不会按照table布局，使用ng-table-include-replace指令来替换ng-include
        template: '<ng-include src="getTmpUrl()" ng-table-include-replace></ng-include>',
        scope: true,
  ...
  (function () {
    ...
    function ngTableSorterRowController($scope) {
      ...
      $scope.getTmpUrl = getTmpUrl;

      ///////////

      function getTmpUrl() {
        return $scope.sortTemplate || 'ng-table/sorterRow.html';
      }
  ...
  (function () {
    ...
    function ngTableSorterRow() {
      ...
      function link(scope, element, attrs) {
        if (attrs.template) {
          scope.sortTemplate = attrs.template;
        }
  ...
  angular.module('ngTable').run([
    '$templateCache', function ($templateCache) {
      ...
      $templateCache.put('ng-table/filters/number.html',
